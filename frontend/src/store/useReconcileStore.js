...
const useReconcileStore = create(
  persist(
    (set, get) => ({
      file: null,
      fileName: '',
      ...
      // Carry-forward state (this will be persisted too via whitelist)
      ...
      toggleRowExpanded: (normalized) => {
        ...
        // Sets don't survive serialization well in persist middleware, 
        // but we'll manually handle or just let it reset on refresh
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        reconcileResults: state.reconcileResults,
